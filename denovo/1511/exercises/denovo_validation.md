...
In this exercise we will perform a full assembly valiadation of 5 assemblies.
In order to facilitate the tutorial all steps have been precomputed and can be found here:

```
$ /proj/g2014179/assemblyValidation/AV_Exercise_executed
```

We will go through all the steps using the Velvet assembly. All assemblies results have been already computed or can be easily computed using a shell script present in each working directory. However you are free to rerun them completely (when the time allows it) or you can run the various steps on your own assemblies.

The excercise is based on real data freely available here:
...
In `/proj/g2014179/assemblyValidation/AV_Exercise_executed/` you can find an identical directory structure with all the exercise already computed. Use this if you want to have a look at the results.
...
FRCbam=/proj/g2014179/assemblyValidation/tools/FRCbam/src/
QAtools=/proj/g2014179/assemblyValidation/tools/qaTools/
REAPR=/proj/g2014179/assemblyValidation/tools/Reapr_1.0.16/
export PATH=$PATH:$FRCbam:$QAtools:$REAPR
export PERL5LIB=/proj/g2014179/assemblyValidation/tools/File-Copy-Link-0.112/lib/
...
```
Rscript --vanilla /proj/g2014179/assemblyValidation/tools/scripts/plotCovGC.R abyss.cov.gc 100
Rscript --vanilla /proj/g2014179/assemblyValidation/tools/scripts/plotCovGC.R allpaths.cov.gc 200
Rscript --vanilla /proj/g2014179/assemblyValidation/tools/scripts/plotCovGC.R masurca.cov.gc 170
Rscript --vanilla /proj/g2014179/assemblyValidation/tools/scripts/plotCovGC.R soapdenovo.cov.gc 100
Rscript --vanilla /proj/g2014179/assemblyValidation/tools/scripts/plotCovGC.R velvet.cov.gc 50
```
